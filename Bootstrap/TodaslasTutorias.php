...
<body style="background-color: #FFFEF7;">
    <nav class="navbar navbar-expand-sm navbar-dark fixed-top" style="background-color: #00355A;">
        ...
        <div class="collapse navbar-collapse" id="collapsibleNavbar">
            ...
            <ul class="navbar-nav ml-md-auto">
                <li>
                    <div class="mr-auto" style="display: inline-flex; padding-top: 6px;">
    ...
    <nav aria-label="breadcrumb">
        <ol class="migas_de_pan breadcrumb" style="background-color: #FFFEF7;">
            ...
            <li class="breadcrumb-item active" aria-current="page"><?php echo $mostrar_todos; ?></li>
        </ol>
    </nav>
    <div class="container">
        <h1 class="titulo1 titulo_profesores"><?php echo $todos; ?></h1>
        <ul class="lista_sin_estilo">
            <?php
            require_once('loginBD.php');
            // Realizamos la consulta SQL
                // p.nombre: columna nombre de la tabla profesores
                // p.apellidos: columna apellidos de la tabla profesores
                // a.nombre_asignatura: columna nombre_asignatura de la tabla asignaturas
                // ap.horario_tutorias: columna horario_tutorias de la tabla asignaturas_profesores
            $sql_tutorias = "SELECT p.nombre, p.apellidos, a.nombre_asignatura, ap.horario_tutorias
                    FROM profesores p
                    JOIN asignaturas_profesores ap ON p.id = ap.id_profesor
                    JOIN asignaturas a ON ap.id_asignatura = a.id";
           // Comprobamos la carga
            if (mysqli_query($conn, $sql_tutorias)) {
                $result_tutorias = mysqli_query($conn, $sql_tutorias);
            }
            else {
                echo "Error: ".$sql_tutorias ."<br>".mysqli_error($conn);
            }
            // Verificamos si hay resultados
            if ($result_tutorias->num_rows > 0) {
                echo "<table border='1'>
                        <tr>
                            <th>Nombre</th>
                            <th>Apellidos</th>
                ...
                while ($row_tutorias =mysqli_fetch_array($result_profesores)) {
                    echo "<tr>
                            <td>" . $row["nombre"] . "</td>
                            <td>" . $row["apellidos"] . "</td>
                            <td>" . $row["nombre_asignatura"] . "</td>
                            <td>" . $row["horario_tutorias"] . "</td>
                        </tr>";
                }
                echo "</table>";
            } else {
