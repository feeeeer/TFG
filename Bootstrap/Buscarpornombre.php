  ...
  <body style="background-color: #FFFEF7;">
    ...
    <nav aria-label="breadcrumb">
      <ol class="migas_de_pan breadcrumb" style="background-color: #FFFEF7;">
        ...
        <li class="breadcrumb-item active" aria-current="page"><?php echo $por_nomb; ?></li>
      </ol>
    </nav>
    <div class="container">
      ...
      ?>
      <form action="Buscarpornombre.php" method="get">
        <div class="form-group">
          <label for="entradaTexto"><?php echo $buscar_nombre; ?></label>
          <input required type="search" class="form-control" id="entradaTexto" name="busqueda" placeholder="<?php echo $escribir_nom; ?>">
        </div>
      ...
      <ul class="lista_sin_estilo lista_busqueda">
        ...
        if (!empty($busqueda)) {
          ...
          $sql = $conn->prepare("SELECT * FROM profesores WHERE otro_departamento = '0' AND nombre_completo LIKE ? ORDER BY nombre_completo ASC");
          $sql->bind_param('s', $busqueda);
          $sql->execute();
          $res = $sql->get_result();

          $cont = 0;
          while ($row_profesores =mysqli_fetch_array($res)) {
            $id = str_replace("@upm.es", "", $row_profesores["correo_upm"]);
            echo '<li>
                    <a href="Profesor.php?codigo_profesor='.$id.'">'.utf8_encode($row_profesores["nombre"])." ".utf8_encode($row_profesores["apellidos"]).'</a>
                  </li>';
            $cont++;
          }
